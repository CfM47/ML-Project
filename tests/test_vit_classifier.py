...
def test_vit_classifier() -> None:  # noqa: D103
    ...
    image = np.random.randint(0, 255, (512, 512), dtype=np.uint8).astype(np.float32) / 255.0
    ...
    try:
        ...
        print(f"Classification result: class={class_label}, confidence={confidence:.4f}")
        
        # Verify output format
        assert isinstance(class_label, (int, np.integer)), "class_label should be int"
        assert isinstance(confidence, (float, np.floating)), "confidence should be float"
    ...
    try:
        rgb_image = np.random.randint(0, 255, (512, 512, 3), dtype=np.uint8).astype(np.float32) / 255.0
        ...
        class_label, confidence = rgb_classifier.classify(rgb_image, x, y, width, height)
        print(f"RGB Classification result: class={class_label}, confidence={confidence:.4f}")
