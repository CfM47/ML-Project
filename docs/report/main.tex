...
\title{Segmentación de Zonas de Fractura en Imágenes SEM: \\
       Comparación de Estrategias mediante un Enfoque AutoML}
%
%
...
\section{Metodología Experimental y Pipeline AutoML}
La resolución del problema de segmentación se abordó mediante un diseño experimental modular que garantiza la reproducibilidad y la comparabilidad entre distintas configuraciones. La metodología establece un flujo de trabajo claro para el procesamiento de datos, entrenamiento y evaluación, permitiendo aislar el impacto de cada componente en el desempeño final.

\subsection{Diseño del Pipeline de Selección}

El núcleo metodológico se basa en un pipeline lineal de procesamiento, donde los datos fluyen secuencialmente a través de etapas bien definidas:

\begin{equation}
    \begin{aligned}
        \text{Imágenes de Fractura} & \rightarrow \text{Preprocesamiento y Aumentación} \rightarrow \text{Segmentación Semántica} \\ 
        &\rightarrow \text{Mapas de Zonas Dúctil/Frágil} \rightarrow \text{Evaluación de Métricas}
    \end{aligned}
\end{equation}

Esta estructura asegura que todas las estrategias de segmentación sean evaluadas bajo las mismas condiciones.

\subsection{Estrategia de Aumentación y Validación}
La primera etapa define el protocolo de datos. Se implementa una estrategia de partición rigurosa (validación cruzada o split fijo) y se aplican técnicas de aumentación de datos de manera controlada. Esto garantiza que todos los modelos vean exactamente las mismas distribuciones de datos durante el entrenamiento y sean evaluados sobre el mismo conjunto de validación, evitando fugas de información.

\subsection{Protocolo de Entrenamiento}
Cada arquitectura de segmentación (ViT, Swin, Quadtree) se entrena siguiendo un protocolo estandarizado. Se encapsula la lógica de entrenamiento para mantener interfaces consistentes, permitiendo comparar modelos de naturaleza muy distinta (e.g., redes neuronales profundas vs. algoritmos recursivos) bajo un marco común.

\subsection{Evaluación Sistemática}
La etapa final consiste en el cálculo de métricas de desempeño sobre las predicciones. Este proceso es independiente del entrenamiento y utiliza un conjunto de métricas estándar para cuantificar la calidad de la segmentación. La separación entre inferencia y evaluación asegura que los resultados sean imparciales y directamente comparables.

Este diseño experimental sistemático permite explorar el espacio de configuraciones (arquitecturas, hiperparámetros, estrategias de aumentación) de manera ordenada, fundamentando la selección del mejor modelo en evidencia empírica robusta.
...
En esta sección se presentan los resultados obtenidos a partir de las distintas combinaciones de estrategias de aumentación y modelos de segmentación evaluados. El desempeño se reporta mediante el F1-score macro y el tiempo total de ejecución, ambos agregados sobre las validaciones correspondientes.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{progression.png}
...
Para complementar los resultados cuantitativos, se realizó una evaluación visual del modelo final Swin Transformer (entrenado con el dataset completo aumentado) sobre un conjunto de prueba independiente. La Figura~\ref{fig:test_predictions} presenta una comparativa cualitativa de las segmentaciones generadas.

En la visualización se muestran tres columnas: a la izquierda, la imagen SEM original; en el centro, la máscara de referencia manual (Ground Truth); y a la derecha, la segmentación predicha por el modelo. Se observa una alta correspondencia estructural entre las predicciones y las anotaciones humanas, destacando la capacidad del modelo para delinear correctamente las fronteras entre zonas frágiles y dúctiles, incluso en áreas de transición compleja. Estos resultados cualitativos respaldan las métricas obtenidas y confirman la aplicabilidad práctica de la solución propuesta.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth, height=0.95\textheight, keepaspectratio]{test_predictions.png}
